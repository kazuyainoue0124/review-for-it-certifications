  ...
  describe 'ポジション（職種）モデルのバリデーションチェック' do
    ...
    context 'ポジション（職種）データが無効なとき' do
      it '名前がなければ無効である' do
        ...
        expect(position_without_name.errors[:name]).to include('を入力してください')
      end

      it '名前が51文字以上なら無効である' do
        position_with_51_characters_name.valid?
        expect(position_with_51_characters_name.errors[:name]).to include('は50文字以内で入力してください')
      end

      it '名前が重複しているなら無効である' do
        expect(position_with_duplicate_name1).to be_valid
        position_with_duplicate_name2.valid?
        expect(position_with_duplicate_name2.errors[:name]).to include('はすでに存在します')
      end
    end
  end
end
